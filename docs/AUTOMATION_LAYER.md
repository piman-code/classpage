...
2. Apps Script가 응답 시트를 읽습니다.
3. 규칙 기반으로 `class-summary.json`과 `lesson-summary.json`을 계산합니다.
4. 결과를 Drive 파일로 쓰거나, `doGet`으로 JSON 응답을 제공합니다.
5. classpage는 그 JSON을 읽기만 합니다.
