...
`최근 며칠 동안 내가 친구에게 도움을 준 경험` 응답이 일정 길이 이상이면 후보로 올립니다.
...
현재 공개 수업용 폼 기준으로는 `오늘 수업에 대한 전체 이해도`, `가장 어려웠던 부분`, `가장 잘 이해한 부분`을 읽습니다.
구버전 시트에 `개념 1`, `개념 2`와 각 이해도 열이 남아 있으면 `Code.gs` fallback으로 함께 읽습니다.
