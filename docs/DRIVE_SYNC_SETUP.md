...
`Google Form -> Google Sheets -> Google 로그인 이메일 기준 집계 -> Apps Script -> Google Drive 폴더 -> Mac 동기화 폴더 -> Obsidian 볼트 classpage-data -> classpage 교사용 페이지`
...
- `classpage-data/`와 동기화 로그는 공개 저장소에 커밋하지 않는 편이 안전합니다.
- 가능하면 Obsidian 볼트와 동기화 대상 폴더를 플러그인 저장소와 분리해서 운영합니다.

## 1. 추천 자동 연동 구조

가장 단순하고 안정적인 운영 방식은 아래입니다.

1. Apps Script가 `refreshAllSummaries()`로 `class-summary.json`, `lesson-summary.json`, `star-ledger.json`을 Google Drive 폴더에 갱신합니다.
...
로그인 이메일 기준 식별과 선택형 allowlist 보완은 Apps Script 단계에서 이미 끝나므로, Mac 자동 복사 쪽에서는 추가 검증 없이 파일만 안정적으로 옮기면 됩니다.
...
아직 Apps Script 설정이 끝나지 않았다면 [docs/BEGINNER_SETUP.md](./BEGINNER_SETUP.md) 의 6단계부터 16단계까지 먼저 끝내는 편이 좋습니다.
빠른 순서만 먼저 보고 싶다면 [docs/START_HERE.md](./START_HERE.md)를 먼저 확인해도 됩니다.
...
- [Config.gs](../automation/apps-script/Config.gs) 의 `output.driveFolderId`

### 정상이라면 보이는 결과

- `refreshAllSummaries()` 실행 후 Drive 폴더 안에 `class-summary.json`
- `refreshAllSummaries()` 실행 후 Drive 폴더 안에 `lesson-summary.json`
- `refreshAllSummaries()` 실행 후 Drive 폴더 안에 `star-ledger.json`
...
4. `class-summary.json`, `lesson-summary.json`, `star-ledger.json` 이 실제로 내려오는지 확인합니다.
...
- [sync_classpage_json.sh](../automation/mac/sync_classpage_json.sh)
...
- [com.classpage.sync-json.plist.example](../automation/mac/com.classpage.sync-json.plist.example)
...
1. [docs/START_HERE.md](./START_HERE.md)와 [docs/BEGINNER_SETUP.md](./BEGINNER_SETUP.md)로 수동 연결을 먼저 성공시킵니다.
...
7. [sync_classpage_json.sh](../automation/mac/sync_classpage_json.sh) 의 두 경로를 수정합니다.
