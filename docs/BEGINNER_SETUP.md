...
`Google Form -> Google Sheets -> Google 로그인 이메일 기준 집계 -> Apps Script -> JSON -> classpage 선생님 페이지`
...
- `class-summary.json`, `lesson-summary.json`, `star-ledger.json` 같은 raw 집계 파일은 선생님용 내부용으로 보고 학생 공개 자료와 분리하는 편이 안전합니다.
...
2. 학생용 페이지와 선생님 페이지가 둘 다 열리는지 확인합니다.
...
7. `classpage-data/*.json`을 볼트에 넣고 선생님 페이지에서 `연결됨` 상태를 확인합니다.

각 단계에서 바로 확인할 곳은 아래입니다.

- 설치 직후
  학생용 페이지에 오늘의 할 일/공지/버튼이 보이고, 선생님 페이지에는 빈 상태 안내가 보이면 정상입니다.
- Form 연결 직후
...
- classpage 연결 직후
  선생님 페이지의 집계 연결 상태가 `연결됨`으로 바뀌면 정상입니다.
...
4. classpage 선생님 페이지에서 표시 확인

이 경로가 되면 그다음에 Drive 동기화나 운영 자동화를 붙이면 됩니다.

처음에는 아래까지만 되어도 충분합니다.

- 학급 집계 또는 수업 집계 중 하나가 선생님 페이지에 보임
- 선생님 페이지의 빈 상태 문구가 왜 비어 있는지 이해됨
- 별점 수동 조정은 Google Sheets에서 한다는 경계를 알고 있음

처음 실험에서 꼭 기억할 것:

- 세 집계 파일을 한 번에 모두 붙이지 않아도 됩니다.
- 학급 집계 또는 수업 집계 하나만 실제로 보여도 첫 연결은 성공입니다.
- 별점은 나중에 붙여도 됩니다. 수동 조정 입력은 여전히 Google Sheets에서 합니다.
...
7. classpage 선생님 페이지에서 집계 결과와 별점모드가 보이는지 확인합니다.

## 0-3. 이 다음에 어떤 문서를 보면 되는가

- 연결 순서만 빠르게 보고 싶다면 [docs/START_HERE.md](./START_HERE.md)를 봅니다.
- 구조를 이해하고 싶다면 [docs/OPERATING_MODEL.md](./OPERATING_MODEL.md)를 봅니다.
- 집계 규칙을 바꾸고 싶다면 [docs/AUTOMATION_LAYER.md](./AUTOMATION_LAYER.md)를 봅니다.
- 다음 단계 제품 방향을 보고 싶다면 [docs/NEXT_STAGE_ROADMAP.md](./NEXT_STAGE_ROADMAP.md)를 봅니다.

## 0-4. 지금 이 플러그인 안에서 되는 일과 아직 안 되는 일

지금 classpage 안에서 할 수 있는 일:

- 학급/수업/별점 JSON이 제대로 들어왔는지 확인
- 수업 화면에서 과목, 단원, 최근 수업 범위, 날짜, 수업 그룹을 바꿔 보며 다음 피드백 대상을 찾기
- CSV 파일 선택 또는 표 붙여넣기로 학생 명단 JSON 만들기
- 별점 화면에서 규칙 현황, 학생별 누적, 최근 이벤트, 숨김 조정 반영 여부를 읽기 전용으로 확인

지금 classpage 안에서 아직 하지 않는 일:
...
초보자 기준으로는 여기까지 기억하면 충분합니다.

- classpage는 집계 파일을 읽어 보여주는 화면입니다.
- 수동 조정 입력은 플러그인 안이 아니라 Google Sheets에서 합니다.
- 선생님 화면이 비어 있으면 먼저 파일 경로와 집계 생성 여부를 확인합니다.
...
`classpage` 선생님 페이지는 이 JSON 파일만 읽습니다.
...
4. 상단 전환 버튼으로 `선생님 페이지`도 눌러봅니다.

### 정상이라면 보이는 결과

- 학생용 페이지에서 오늘의 할 일, 공지사항, 제출 바로가기 카드가 보입니다.
- 선생님 페이지에서는 아직 JSON이 없더라도 빈 상태 안내 카드가 보입니다.

### 자주 틀리는 부분

- 플러그인은 설치했지만 활성화하지 않음
- 리본 아이콘이 안 보일 때 명령 팔레트를 시도하지 않음
- 선생님 페이지에 데이터가 안 보이는 것을 설치 실패로 오해함
...
- [Config.gs](../automation/apps-script/Config.gs)
- [Code.gs](../automation/apps-script/Code.gs)
- [appsscript.json](../automation/apps-script/appsscript.json)
...
선생님 전용 조정까지 쓰고 싶을 때만 `별점 수동 조정` 시트를 추가하면 됩니다.

중요:

- 수동 조정 입력 위치는 지금도 Google Sheets `별점 수동 조정` 시트입니다.
- classpage는 그 결과가 `star-ledger.json`에 반영되었는지 읽어서 보여주는 화면입니다.
...
- 학생 명단 JSON 경로: `classpage-data/student-roster.json`

### 선택: 학생 명단 연결하기

학생 명단 JSON은 `누가 아직 제출하지 않았는지` 보기 위한 기준표입니다.

중요한 점:

- Google Form을 제출하지 않은 학생은 `studentResponses`에 아예 안 나타날 수 있습니다.
- 그래서 명단이 있어야 `응답이 없는 학생`도 선생님 화면에서 `미제출`로 잡아낼 수 있습니다.
- 명단을 연결하지 않아도 기존 학급/수업/별점 화면은 그대로 동작합니다.
...
- `Settings -> classpage -> 학생 명단 JSON (선택) -> 학생 명단 JSON 경로`

가장 쉬운 방법:

1. `Settings -> classpage -> 학생 명단 가져오기 도우미`로 들어갑니다.
2. `CSV 파일 선택`으로 명단 CSV를 불러오거나, `CSV 내용 붙여넣기` 칸에 표를 붙여넣습니다.
3. 반 컬럼이 없다면 `기본 학급 (선택)`에 예: `3-2`를 넣습니다.
4. `가져오기 미리보기`를 눌러 읽은 학생 수와 제외된 행을 먼저 확인합니다.
5. 결과가 맞으면 `명단 JSON 저장`을 눌러 `student-roster.json`으로 저장합니다.
6. 저장이 끝나면 그 명단은 바로 선생님 화면의 미제출 학생 비교에 사용되고, `학생 명단 JSON 경로`도 그 파일로 함께 맞춰집니다.

CSV를 준비할 때 필요한 컬럼:

- 필수: `classroom` 또는 `class` 또는 `반` 또는 `학급`
- 필수: `number` 또는 `no` 또는 `번호`
- 필수: `name` 또는 `이름` 또는 `학생명`
- 선택: `studentId` 또는 `학번`
- 선택: `note` 또는 `비고` 또는 `메모`
...
엑셀이나 구글 시트에서 바로 쓰고 있다면:

- CSV로 저장한 뒤 `CSV 파일 선택`을 눌러도 됩니다.
- 또는 학생 표를 그대로 복사해서 `CSV 내용 붙여넣기` 칸에 붙여넣어도 기본적으로 읽을 수 있습니다.
- 이번 버전은 CSV를 우선 지원합니다. XLSX 파일 직접 불러오기는 아직 없습니다.

직접 JSON으로 연결하고 싶다면:

- 기존처럼 `student-roster.json`을 손으로 만들어 `학생 명단 JSON 경로`에 바로 연결해도 됩니다.
- 아래 JSON 예시는 `직접 JSON을 준비할 때` 참고하는 예시입니다.
- CSV 가져오기 예시는 `docs/contracts/student-roster.example.csv`, JSON 예시는 `docs/contracts/student-roster.example.json`을 보면 덜 헷갈립니다.

가장 단순한 형식은 아래처럼 쓰면 됩니다.

```json
{
  "type": "student-roster",
  "generatedAt": "2026-03-10T07:00:00+09:00",
  "sourceLabel": "2026학년도 3학년 2반 명단",
  "defaultClassroom": "3-2",
  "students": [
    { "classroom": "3-2", "number": "01", "name": "학생A" },
    { "classroom": "3-2", "number": "07", "name": "학생B", "studentId": "2026-3-2-07" },
    { "classroom": "3-2", "number": "12", "name": "학생C", "note": "전학 후 첫 주" }
  ]
}
```
...
- JSON에서는 `defaultClassroom`을 넣어 두고 학생별 `classroom`을 생략해도 됩니다.
- 하지만 처음에는 예시처럼 `classroom`, `number`, `name`을 함께 적는 편이 덜 헷갈립니다.

추가로 아래 별칭도 읽을 수는 있지만, 처음에는 권장 필드 이름 그대로 쓰는 편이 덜 헷갈립니다.

- `classNumber` -> `classroom`
- `studentNumber` -> `number`

미제출 학생은 아래 기준으로 찾습니다.

- 먼저 `classroom + number + name`을 기준으로 비교합니다.
- 공백 차이와 표기 차이는 어느 정도 정리해서 비교합니다.
- 예: `3-2` 와 `3학년 2반`
- 예: `07` 과 `7` 과 `7번`
- 현재 수업 화면에서는 `현재 선택한 수업 그룹`의 응답과 비교합니다.
- 학급 화면에서는 `학급 집계`의 응답과 비교합니다.

현재 버전의 한계:

- XLSX 파일 직접 불러오기는 아직 없습니다.
- CSV 붙여넣기와 CSV 파일 불러오기를 우선 지원합니다.
- Google Sheets와 자동 양방향 연결도 아직 없습니다.
- 같은 반에 같은 번호/이름이 중복되면 수동 확인이 필요할 수 있습니다.

- 더 긴 예시는 `docs/contracts/student-roster.example.json` 과 `docs/contracts/student-roster.example.csv` 에도 있습니다.

### 선택: 학생 사진 매핑 연결하기

학생 사진은 집계 JSON에 넣지 않고 별도 JSON 파일로 연결할 수 있습니다.
...
- `Settings -> classpage -> 학생 사진 매핑 (선택) -> 학생 사진 매핑 JSON 경로`

가장 단순한 형식은 아래처럼 `classroom|number|name` 를 key로 쓰는 JSON 객체입니다.

```json
{
  "3-2|1|김민서": "classpage-assets/student-photos/3-2-01-kim-minseo.jpg",
  "3-2|2|박준호": "classpage-assets/student-photos/3-2-02-park-junho.png",
  "3-2|15|이서윤": "./student-photos/3-2-15-lee-seoyoon.webp"
}
```

- key는 집계에 들어오는 학생 식별값과 같은 순서인 `반|번호|이름` 입니다.
- value는 Obsidian 볼트 기준 상대 경로입니다.
- `./` 로 시작하면 매핑 JSON 파일이 들어 있는 폴더 기준 상대 경로로 읽습니다.
- 예시는 `docs/contracts/student-photo-map.example.json` 에도 있습니다.
- 사진이 없거나 경로가 틀리면 선생님 화면에서는 자동으로 이니셜 아바타로 대신 표시됩니다.

### 정상이라면 보이는 결과

- 선생님 페이지에서 집계 연결 상태가 보입니다.
- 학급 집계 카드가 보입니다.
- 수업 집계 카드가 보입니다.
- 별점모드 카드가 보입니다.
- 학생 명단을 연결했다면 `학생 명단 파일` 상태가 `연결됨`으로 보입니다.
- 학생 명단과 집계가 둘 다 있으면 `아직 제출하지 않은 학생` 카드가 함께 보입니다.
...
- CSV 첫 줄 헤더를 빼먹음
- CSV에 반/번호/이름 중 하나가 빈 행이 섞여 있음
- 같은 학생이 CSV에 두 번 들어 있어 일부가 제외됨
- `student-roster.json`의 `type`을 빼먹음
- 명단 JSON의 `classroom` 값과 집계 JSON의 학급 표기가 너무 다름
...
### 16-5. 선생님 화면 보기 기준 바꾸기

선생님마다 먼저 보고 싶은 학생 기준이 다를 수 있습니다.  
그래서 classpage는 선생님 화면에서 `보기 프리셋`과 `학생 목록 정렬`을 직접 바꿀 수 있습니다.

설정 위치:

- `Settings -> classpage -> 선생님 화면 보기 기준`
...
- `기본형`
  가장 균형 있게 보는 기본 설정입니다. 위험 학생, 미제출 학생, 칭찬 후보를 너무 한쪽으로 치우치지 않게 봅니다.
- `위험 조기 발견형`
  도움이 필요한 학생, 수업 후속지도, 미제출 학생을 더 먼저 봅니다.
- `칭찬 강화형`
  칭찬/격려 후보와 최근 긍정 흐름을 더 먼저 봅니다.
- `미제출 집중형`
  아직 제출하지 않은 학생을 가장 먼저 확인하기 좋게 둡니다.

#### 어떤 선생님에게 어떤 프리셋이 잘 맞을까

- 아침 조회나 수업 시작 전에 빠르게 위험 학생부터 보고 싶다면: `위험 조기 발견형`
- 교실 분위기를 살리고 칭찬을 자주 활용하고 싶다면: `칭찬 강화형`
- 폼 제출 독촉과 확인이 자주 필요하다면: `미제출 집중형`
- 아직 익숙하지 않아서 무난한 화면이 필요하다면: `기본형`

#### 학생 목록 기본 정렬이 바뀌면 무엇이 달라질까

- `번호순`
  출석부처럼 찾기 쉽습니다. 수업 시간에 바로 부르기 편합니다.
- `위험 우선`
  도움이 필요한 학생, 후속지도가 필요한 학생이 더 앞쪽에 옵니다.
- `칭찬 우선`
  칭찬/격려할 학생이나 긍정 흐름이 보이는 학생이 더 앞쪽에 옵니다.
- `최근 반영 순`
  최근 집계에 먼저 반영된 순서를 더 유지합니다.

#### 강조를 켜면 어떻게 달라질까

- `미제출 학생 강조`
  overview와 학급/수업 화면에서 미제출 학생 카드가 더 앞쪽에 오거나 더 먼저 눈에 들어옵니다.
- `위험 학생 강조`
  `지금 먼저 볼 학생`, `수업 후속지도 우선`, `도움이 필요한 학생` 흐름이 더 앞쪽에 옵니다.
- `칭찬/격려 후보 강조`
  칭찬/격려 카드와 관련 학생 목록이 더 먼저 보입니다.

#### 미제출 강조를 켰는데 명단이 없으면?

- 설정 자체는 그대로 보입니다.
- 다만 학생 명단 JSON이 없으면 실제 미제출 비교는 할 수 없습니다.
- 이 경우 선생님 화면에 `학생 명단 가져오기 도우미에서 CSV를 저장하거나 학생 명단 JSON을 연결하면 미제출 강조가 함께 동작합니다.` 같은 안내가 함께 보입니다.

#### 이 버전에서 되는 것 / 아직 안 되는 것

지금 바로 되는 것:

- 집계 JSON 읽기
- 학생 명단 JSON 연결 후 미제출 학생 표시
- CSV 기반 학생 명단 가져오기 도우미
- 학생 사진 매핑 또는 이니셜 아바타 fallback
- 프리셋, 정렬, 강조 토글로 선생님 화면 보기 기준 바꾸기
...
- 학생 명단 JSON 경로
  `Settings -> classpage -> 학생 명단 JSON (선택) -> 학생 명단 JSON 경로`
- 학생 명단 가져오기 도우미
  `Settings -> classpage -> 학생 명단 가져오기 도우미`
- 보기 프리셋/정렬/강조
  `Settings -> classpage -> 선생님 화면 보기 기준`
...
- 선생님 페이지에서 집계 파일 상태가 `연결됨`으로 바뀝니다.
...
수동 복사까지 성공한 뒤 Drive 자동 갱신 + Mac 자동 복사 방식으로 넘어가려면 [docs/DRIVE_SYNC_SETUP.md](./DRIVE_SYNC_SETUP.md) 를 이어서 보면 됩니다.
